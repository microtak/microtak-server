...
Test-case catalog for MicroTAK, derived from source-level research on the official TAK Server, `tkuester/taky`, and `brian7704/OpenTAKServer`, plus this project's own design decisions for the mesh-sync layer. Every feature is expected to be backed by a test (unit and/or API-level integration test) before being considered done — this document is the checklist that's measured against.
...
- **[COMPAT]** — a confirmed real-server/real-client behavior MicroTAK must match for interoperability. Cited to a primary or strongly-corroborated source.
- **[DESIGN]** — a behavior with no authoritative spec found; MicroTAK is making its own decision and the test verifies *that decision*, not compatibility with anything external.
- **[HARDEN]** — a defensive/security test targeting a known weakness or bug class found in an existing implementation, which MicroTAK should deliberately avoid repeating.
...
| TC-COT-09 | Enforce a configurable maximum single-event size and reject (not silently OOM) an oversized `<event>`. At least one reference implementation has **no such limit** (an explicit open TODO in its own source) — this is a deliberate MicroTAK improvement, not a compatibility requirement. | [HARDEN] | ❌ not implemented |
...
| ID | Case | Type |
|---|---|---|
| TC-TLS-01 | Accept a valid client cert signed by MicroTAK's own CA over the mTLS CoT port | [COMPAT] |
| TC-TLS-02 | Reject a client cert not signed by MicroTAK's CA (untrusted issuer) at the TLS layer | [COMPAT] |
| TC-TLS-03 | Reject a client with no cert at all when client-cert-required mode is on; decide deliberately whether MicroTAK offers an unauthenticated mode at all | [DESIGN] |
...
| TC-ENROLL-04 | Re-enrollment of an already-enrolled CN (same identity requests a new cert) is handled per a documented policy — no authoritative source found for official-server behavior here, so this is MicroTAK's own decision | [DESIGN] |
...
Real official-server error-status contracts for this area are undocumented by any authoritative public source found. Test cases marked [DESIGN] here are MicroTAK's own contract, not verified compatibility targets.

**Implemented** (`src/missions.rs` + `src/marti/missions.rs`): mission metadata CRUD, change log, subscriptions, and content references — covers TC-MARTI-01, 02, 03, 04, 05, 06, 11, and 12, each with both a store-level unit test and (for the core lifecycle) a real HTTP integration test via `tower::ServiceExt::oneshot`, plus a full lifecycle exercised over real HTTP against the running server in `tests/e2e.rs`. TC-MARTI-02/12 resolve MicroTAK's own contract concretely: `PUT` is strict-create-only (409 on a duplicate name), `PATCH` is the update path and only touches provided fields — deliberately stricter than a reference implementation's merge-on-`PUT` behavior. TC-MARTI-06's concurrency policy (serialize via a process-wide write lock, last-writer-wins per field, no torn writes) is verified with 20 concurrent threads updating the same mission. TC-MARTI-11 is satisfied by construction: mission names are never sanitized anywhere, used verbatim as an opaque key, so the store-once-differently/lookup-differently bug class can't occur. **TC-MARTI-10 is now implemented** (`src/marti/mod.rs`'s `MtlsHttpServer`, hand-rolled on `hyper-util` + `tokio-rustls` since `axum::serve` only binds a plain `TcpListener`): the missions API requires a client cert signed by the CA for every connection, reusing the exact same `ServerConfig`/verification policy as the CoT mTLS relay. Covered by two real end-to-end tests in `tests/e2e.rs` — a request with no client cert fails the handshake, and a request with a cert from a foreign CA also fails — both against the real, running server, driven through a real `reqwest` mTLS client (with `.resolve()` used to make the server cert's fixed SAN hostname resolvable to the test's loopback address, and `.no_proxy()` to bypass the sandbox's own outbound HTTP proxy, which otherwise intercepts requests to that fake hostname). **TC-MARTI-10's residual gap above is now closed** (`src/marti/missions.rs`'s `require_matching_identity`): every handler that accepts a `creatorUid`/`actorUid`/`uid` claim rejects with 403 if it doesn't match the authenticated `PeerIdentity`, covered by module-level tests and two `tests/e2e.rs` cases. **TC-MARTI-09 is now implemented** (`src/transport/connections.rs`'s `ConnectedClients`, `src/marti/client_endpoints.rs`): `TcpRelay` and `TlsRelay` both register a live entry on connect (address, transport, cert CN when mTLS, bound uid once TC-TLS-04 binding succeeds) and unregister via an RAII guard on every disconnect path, so `GET /Marti/api/clientEndPoints` always reflects who's actually connected rather than a static list — verified with a module-level router test and a real end-to-end test in `tests/e2e.rs` that connects a plain-TCP and an mTLS client against the running server, asserts both appear with the right shape, disconnects them, and asserts the list goes empty again. **TC-MARTI-07/08 are now implemented** (`src/content_store.rs`'s `ContentStore`, `src/marti/content.rs`): file content is stored hash-addressed under `data_dir/content/`, keyed by the server's own computed SHA-256 regardless of what a caller claims (TC-MARTI-07) — a claimed hash that doesn't match is rejected with 400 before anything touches disk — and written via temp-file + `fsync` + atomic rename so a crash never leaves a partially-written file visible under its final content-addressed name (TC-MARTI-08). A caller-supplied `hash` query param is validated as well-formed hex before any filesystem lookup, closing a path-traversal opening an unvalidated value would otherwise leave. Exposed as `POST /Marti/api/sync/missionupload` and `GET /Marti/api/sync/content?hash=<h>`, deliberately kept decoupled from `MissionContentRef` metadata (same separation this module's own doc comment already describes) — verified with module-level store and router tests plus a real end-to-end upload→reference→download round trip in `tests/e2e.rs`.

| ID | Case | Type |
|---|---|---|
| TC-MARTI-01 | `PUT /Marti/api/missions/<name>` creates a mission; `GET` retrieves it; `DELETE` removes it | [COMPAT] (endpoint shape) |
| TC-MARTI-02 | Creating a mission with a name that already exists returns a clear, documented error (e.g. `409 Conflict`) — MicroTAK's own contract | [DESIGN] |
...
| TC-MARTI-07 | `GET /Marti/api/sync/content?hash=<h>` — returned file's actual content hash matches the requested hash (a reference implementation does **not** verify this, trusting stored metadata blindly — an integrity gap worth deliberately avoiding) — **implemented** | [HARDEN] |
| TC-MARTI-08 | Data package upload + metadata-sidecar write is atomic (or safely resumable) — a crash mid-upload must not leave orphaned symlinks/partial files that later 500 or serve corrupt content — **implemented** | [HARDEN] |
| TC-MARTI-09 | `GET /Marti/api/clientEndPoints` reflects **actual** currently-connected clients, not a static/hardcoded stub (a confirmed real gap in at least one reference implementation) — **implemented** | [HARDEN] |
| TC-MARTI-10 | Requests to `/Marti/*` correctly require and validate the mTLS client cert per-request, avoiding the class of bug where a reverse-proxy fails to forward cert info to the app layer (a confirmed real deployment trap in more than one reference implementation) — **implemented**, including per-request identity-claim enforcement | [HARDEN] |
...
| TC-ROUTE-03 | On client disconnect, synthesize and broadcast a `t-x-d-d` CoT event (`how="h-g-i-g-o"`) announcing that uid's departure, rather than relying purely on `stale` expiry — no *base-spec* disconnect event type is documented anywhere, but at least one real deployed reference server actively does this, and MicroTAK adopts the same convention | [COMPAT] |
...
| ID | Case | Type |
|---|---|---|
| TC-MESH-01 | Two MicroTAK instances with no direct connectivity, each holding independent CoT state, converge to the same merged state once a sync opportunity (any transport) becomes available | [DESIGN] |
...
**Implemented** (`src/config.rs`, `src/app.rs`): an optional TOML config file (`$MICROTAK_CONFIG` or `./microtak.toml`, every field individually overridable, backed by the same defaults as `AppConfig`) plus real persistence — CA, device registry, and mission store all read from and write to `data_dir` (default `./data`), surviving a restart instead of regenerating on every startup. Verified against the real binary, not just the test suite: a CA's SHA-256 fingerprint was confirmed identical across two actual `microtakd` runs against the same `data_dir`. TC-CFG-01 covers both a cheap type-level check (an out-of-range port fails TOML deserialization itself) and an eager semantic check (`bind_host` must parse as an IP, `cert_validity_days` must be positive) rather than surfacing as a confusing later `SocketAddr`-bind or silently-negative-duration failure. TC-CFG-02's spirit is satisfied the other way around from its literal wording (no external TLS cert/key path exists to reference, since MicroTAK issues its own CA-derived certs) — a *missing* config file is fine (falls back to defaults), a *present-but-invalid* one is a hard startup error, which is the distinction that actually matters here.
...
| TC-CFG-03 | No hardcoded, widely-known default secret ships in MicroTAK's default config — at least one reference implementation's default PKCS12 export password is a literal, widely-known-in-community string. MicroTAK should generate a random one or refuse to start without an explicit one set. | [HARDEN] |
...
## 16. Enrollment lockdown / admin API (new module, not part of the original catalog)

Added 2026-09-23, prompted directly by a permissions/enrollment design discussion: enrollment (`POST /Marti/api/tls/signClient/v2`) is wide open to anyone who can reach it by design (matching the real Marti contract), which is a real gap for any deployment not fully isolated on a trusted network. No external reference implementation to compare against here (this is MicroTAK's own design), so tracked with plain descriptions rather than TC-IDs.

**Implemented** (`src/enrollment_tokens.rs`, `src/marti/admin.rs`), each with a passing unit, module, or end-to-end test: `EnrollmentTokenStore` mints single-use, optionally-expiring tokens, event-log-backed like every other piece of state; a token can only be consumed once (verified with a real multi-threaded contention test — 8 concurrent attempts to consume the same token, exactly one succeeds); a revoked token is rejected even if never used; an expired token is rejected, one issued-but-not-yet-expired is still accepted. A minimal admin API (`/Marti/api/admin/enrollmentTokens`, mTLS-authenticated) mints/lists/revokes tokens, gated by `AppConfig::admin_common_name` — fails closed (no configured admin means the API is unreachable by *anyone*, not open-by-default), verified by mutation-testing the identity check itself.

`AppConfig::enrollment_mode` gates the enrollment endpoint, revised 2026-09-23 from an opt-in boolean (`enrollment_requires_token`, off by default — insecure by default, and required a restart to lock down) to `EnrollmentMode::{Auto, Open}` with `Auto` as the default. `Auto` is evaluated live on every enrollment attempt (`EnrollmentState::is_locked_down`, mutation-tested), not decided once at startup: it requires a token once the configured `admin_common_name` has actually enrolled (checked against the live device registry), and stays open until then — so a fresh install is secure by default without a manual "turn on lockdown" step, and no restart is needed for the transition. `Open` remains available as an explicit, permanent opt-out. Rejected enrollment requests neither register the device nor return the signed cert. A real end-to-end test (`e2e_enrollment_auto_mode_locks_down_the_moment_the_admin_enrolls`) exercises the live transition against a single `App::bind()` (no restart, no data-dir replay trick): an unrelated device enrolls while the admin hasn't enrolled yet (open), the admin then enrolls (also open), and from that exact moment a new device with no token is rejected, a token minted through the real admin API succeeds and works over mTLS, and the token can't be reused.

**Not yet implemented**: multi-use tokens (single-use only, a deliberate simplicity choice for v1). QR-code distribution and a CLI for minting/managing tokens now exist as the separate `microtak-admin-cli` tool (its own repo, its own test suite). Real roles/permissions on missions — the other half of the original permissions/enrollment discussion — is now implemented, see §17 below.

## 17. Mission roles (extends §5, not part of the original catalog)

Added 2026-09-23, closing the second half of the permissions/enrollment discussion §16 opened: missions previously had zero authorization beyond identity-claim matching. No external reference implementation to compare against for the exact policy (real TAK Server/OpenTAKServer's own role names — `MISSION_OWNER`/`MISSION_SUBSCRIBER` — were the inspiration, not a confirmed-compatible wire contract), so tracked with plain descriptions rather than TC-IDs.

**Implemented** (`src/missions.rs`, `src/marti/missions.rs`), each with a passing unit, module, or end-to-end test: `MissionRole` (`Owner`, `Subscriber`) auto-assigned on creation/subscription, auto-removed on unsubscription (an `Owner` who subscribes/unsubscribes from their own mission never loses `Owner`) — verified including the specific "owner subscribes to their own mission" edge case. Role state is deterministically re-derived by the reducer from existing `Created`/`Subscribed`/`Unsubscribed` events rather than stored as new event-log fields, verified to survive a reload exactly (including explicit `assign_role`/`revoke_role` calls, not just the automatic assignments). `delete`/`update` require `Owner`; `add_content` requires any role — each enforcement point mutation-tested (the check physically removed, confirming the corresponding test fails, then reverted) per this project's own red-team review methodology. `list`/`get`/`changes` remain deliberately unrestricted regardless of role. New `PUT`/`DELETE /missions/:name/role(/:uid)` let an `Owner` manage other identities' roles; a last-owner protection (rejecting the demotion/revocation that would leave a mission with zero owners) is enforced at the store level and confirmed to surface as a real `409` through the HTTP layer, including the two-owner case where demoting *one* of two owners is correctly still allowed. A real end-to-end test drives the complete flow through three separately mTLS-enrolled devices (owner, subscriber, outsider) against the fully assembled server.
