...
**Implemented** (`src/enrollment_tokens.rs`, `src/marti/admin.rs`), each with a passing unit, module, or end-to-end test: `EnrollmentTokenStore` mints single-use, optionally-expiring tokens, event-log-backed like every other piece of state; a token can only be consumed once (verified with a real multi-threaded contention test — 8 concurrent attempts to consume the same token, exactly one succeeds); a revoked token is rejected even if never used; an expired token is rejected, one issued-but-not-yet-expired is still accepted. `AppConfig::enrollment_requires_token` (off by default) gates the enrollment endpoint on a valid token when enabled; rejected requests neither register the device nor return the signed cert. A minimal admin API (`/Marti/api/admin/enrollmentTokens`, mTLS-authenticated) mints/lists/revokes tokens, gated by `AppConfig::admin_common_name` — fails closed (no configured admin means the API is unreachable by *anyone*, not open-by-default), verified by mutation-testing the identity check itself. A real end-to-end test exercises the full two-phase bootstrap this design requires (enroll the admin while enrollment is open, "restart" with the lockdown enabled, mint a token through the real admin API, confirm an unrelated device is rejected without one and succeeds with one, confirm the token can't be reused) across two separate `App::bind` calls against the same `data_dir`, simulating an actual restart rather than a single in-process toggle.

**Not yet implemented**: a separate `microtak-admin` tool/QR-code generation for distributing tokens to devices (currently a token is just a string, no UX around it yet); multi-use tokens (single-use only, a deliberate simplicity choice for v1). Real roles/permissions on missions — the other half of the original permissions/enrollment discussion — is now implemented, see §17 below.
