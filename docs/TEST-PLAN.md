...
**Not yet implemented**: real roles/permissions on missions (MISSION_OWNER/MISSION_SUBSCRIBER-style, matching real TAK Server/OpenTAKServer) — the other half of the original permissions/enrollment discussion, deferred as a separate, larger data-model change; a separate `microtak-admin` tool/QR-code generation for distributing tokens to devices (currently a token is just a string, no UX around it yet); multi-use tokens (single-use only, a deliberate simplicity choice for v1).
