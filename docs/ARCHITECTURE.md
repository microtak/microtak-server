...
**Status: early but functional.** `edgetakd` runs a real server: CoT parsing (`src/cot.rs`), a plain-TCP and an mTLS CoT relay sharing one cross-transport broadcast bus (`src/transport/`), a certificate authority with CSR signing (`src/pki.rs`), a device registry with cert-CN↔uid identity binding and revocation (`src/registry.rs`), a certificate enrollment HTTP endpoint (`src/marti/enrollment.rs`), and a mission (Data Sync) metadata API — CRUD, change log, subscriptions (`src/missions.rs` + `src/marti/missions.rs`) — all assembled by `src/app.rs`. Covered end-to-end by `tests/e2e.rs` in addition to each module's own tests. No config file, no persistence across restarts, no DataSync file content storage, and no mesh sync yet — everything below past this point describes target design, not all of it built. **The missions API is not yet mTLS-authenticated** (unlike the CoT relay) — see `marti::missions`'s doc comment; don't expose it beyond a trusted network.
